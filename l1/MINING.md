...
`SHA256SUMS` is part of the reviewed release identity. Its entries are serialized only as deterministic forward-slash paths relative to the canonical release root, never relative to the runner working directory. Parent-directory or absolute-path escapes are rejected, and serialized paths containing ASCII control bytes (including CR, LF, tab, NUL or DEL) fail closed rather than becoming ambiguous or parser-dependent manifest lines. That keeps checksum evidence independent of CI checkout/cwd layout and prevents host-specific prefixes or injected line structure from entering the manifest. The underlying files remain descriptor-bound during hashing and non-regular/symlink entries continue to fail closed.
