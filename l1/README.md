...
- bounded JSON RPC with per-client rate limits, global connection/time/in-flight limits, 64 headers and 100 requests per persistent socket, health/metrics, optional consensus-write authentication, status, blocks, balances, nonces, transaction submission, proposal attestation, and finalized-block acceptance;
