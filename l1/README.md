...
- bounded JSON RPC with per-client rate limits, global connection/time/request-count limits, a 25 MB aggregate request-body budget held through handler completion, a 25 MB queued-response budget held through socket finish/close, 64 headers and 100 requests per persistent socket, separate liveness/fail-closed readiness endpoints, admission and request/response byte-pressure metrics with rejection counters, optional consensus-write authentication, status, blocks, balances, nonces, transaction submission, proposal attestation, and finalized-block acceptance;
- static-peer identity handshake and incremental finalized-block synchronization with JSON/content-length preflight, per-response stream limits, and a 50 MB node-wide in-flight peer-response budget;
...
`l1/config/public-testnet-identity.json` and `l1/config/public-testnet-bootstrap.json` are unfilled public-testnet proposals, not a live network. Bootstrap slots contain the token `PLACEHOLDER` and are not dial targets. `node dist/src/cli.js node --network-class public-testnet` refuses to open a data directory until a reviewed identity matches the genesis file, a reviewed bootstrap set exists, and launch authorization separately allows activation. Those flags stay false, and the bootstrap file cannot mark itself live. Details: [`docs/PUBLIC_TESTNET_READINESS.md`](../docs/PUBLIC_TESTNET_READINESS.md).
