...
test("RPC exposes liveness, readiness, and metrics while enforcing per-client request limits", async () => {
  ...
  try {
    ...
    assert.deepEqual(payload.p2pFrames, {
      inbound: { bytesInUse: 0, maxBytes: 64 * 1_024 * 1_024, peakBytesInUse: 0, rejectedFrames: 0 },
      outbound: { bytesInUse: 0, maxBytes: 64 * 1_024 * 1_024, peakBytesInUse: 0, rejectedFrames: 0 }
    });
