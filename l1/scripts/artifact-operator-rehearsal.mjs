  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (latest.every((status) => status.height >= minHeight) &&
          ...
          new Set(latest.map((status) => status.genesisHash)).size === 1) {
        return latest;
      }
    } catch {
...
try {
  ...
  const nodeOneArgs = [
    ...
    "--host", "127.0.0.1",
    "--port", String(portOne),
    "--validator-key", validatorOnePath,
    "--peer", `http://127.0.0.1:${portTwo}`
  ];
  const nodeTwoArgs = [
    "--genesis", genesisPath,
    "--data", dataTwo,
    "--host", "127.0.0.1",
    "--port", String(portTwo),
    "--validator-key", validatorTwoPath,
    "--peer", `http://127.0.0.1:${portOne}`
  ];
