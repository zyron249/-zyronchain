...
import { constants, openSync, closeSync, fstatSync, lstatSync, readFileSync, readSync, realpathSync } from 'node:fs';
...
// Active publication checks require a no-follow open; unsupported hosts fail closed.
if (typeof constants.O_NOFOLLOW !== 'number') throw new Error('Active publication verification requires O_NOFOLLOW support; use the Linux promotion CI');
const parent = path.dirname(absolutePath);
const parentStat = lstatSync(parent, { bigint: true });
if (!parentStat.isDirectory() || parentStat.isSymbolicLink() || realpathSync(parent) !== path.join(realpathSync(repositoryRoot), 'evidence')) throw new Error('publication parent must be a real repository evidence directory');
...
const fd = openSync(absolutePath, constants.O_RDONLY | constants.O_NOFOLLOW | (constants.O_NONBLOCK || 0));
let workingBytes;
try {
  const opened = fstatSync(fd, { bigint: true });
  if (!opened.isFile() || !sameIdentity(stat, opened) || opened.size > 65536n) throw new Error('publication evidence changed or exceeds 64 KiB');
  ...
  if (count > 65536 || BigInt(count) !== opened.size || !sameIdentity(opened, after) || opened.size !== after.size || opened.mtimeNs !== after.mtimeNs || opened.ctimeNs !== after.ctimeNs || current.isSymbolicLink() || !sameIdentity(opened, current) || !sameIdentity(parentStat, lstatSync(parent, { bigint: true }))) throw new Error('publication evidence changed during verification');
  workingBytes = buffer.subarray(0, count);
