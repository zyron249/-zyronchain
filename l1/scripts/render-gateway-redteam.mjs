...
async function waitFor(url, predicate, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      // Startup and bounded gateway rejection are expected during polling.
    }
...
try {
  ...
  const counts = { 200: 0, 503: 0, errors: 0 };
  const waveSize = 128;
  const waves = 8;
  for (let wave = 0; wave < waves; wave += 1) {
    const results = await Promise.all(Array.from({ length: waveSize }, async (_, index) => {
      ...
      try {
        return await fetchJson(`${baseUrl}${path}`, {}, 6_000);
      } catch (error) {
        return { status: 0, error: error instanceof Error ? error.message : String(error) };
      }
    }));
    for (const result of results) {
      if (result.status === 200) counts[200] += 1;
      else if (result.status === 503) counts[503] += 1;
      else counts.errors += 1;
    }
  }

  assert.equal(counts.errors, 0, `Unexpected gateway responses during burst: ${JSON.stringify(counts)}`);
  assert.equal(counts[200] + counts[503], waveSize * waves);
  ...
  assert.equal(post.status, 405, "Public gateway must remain read-only under attack");
  ...
  console.log(JSON.stringify({
    status: "ok",
    scenario: "render-gateway-bounded-burst",
