...
const allowedMethods = new Set(['public-mining-activation-review', 'release-activation-verification']);
...
function runGit(args, binary = false) {
  ...
  if (result.status !== 0) throw new Error(`unable to resolve public-mining activation evidence from exact source commit: ${(binary ? result.stderr?.toString('utf8') : result.stderr) || args.join(' ')}`);
...
function exactSourceBlob(relativePath) {
  ...
  if (!match || match[3] !== relativePath) throw new Error('publicMiningActivation evidence must be a regular file in exact sourceCommit');
  const bytes = runGit(['show', `${policy.sourceCommit}:${relativePath}`], true);
  if (!Buffer.isBuffer(bytes)) throw new Error('publicMiningActivation exact source blob unavailable');
  return bytes;
}
...
if (!/^[0-9a-f]{40}$/.test(policy.sourceCommit || '')) throw new Error('publicMiningActivation evidence requires exact sourceCommit');
...
if (new Set(subjects.map((s) => s.artifact.name)).size !== platforms.length) throw new Error('public-mining activation subjects must use distinct artifact filenames');
if (new Set(subjects.map((s) => s.artifact.sha256)).size !== platforms.length) throw new Error('public-mining activation subjects must use distinct artifact digests');
...
if (!evidenceDigest) throw new Error('publicMiningActivation evidence must include exact sha256 digest binding');
const exactUrl = `https://github.com/zyron249/-zyronchain/blob/${policy.sourceCommit}/${evidencePath}`;
if (evidence.slice(0, evidence.length - evidenceDigest[0].length) !== exactUrl) throw new Error('publicMiningActivation evidence must bind canonical exact-sourceCommit Git blob');

const absolutePath = path.resolve(repositoryRoot, evidencePath);
if (!absolutePath.startsWith(`${repositoryRoot}${path.sep}`)) throw new Error('publicMiningActivation evidence path escapes repository root');
const stat = lstatSync(absolutePath);
if (stat.isSymbolicLink() || !stat.isFile()) throw new Error('publicMiningActivation evidence must be a regular non-symlink working-tree file');
const workingBytes = readFileSync(absolutePath);
const sourceBytes = exactSourceBlob(evidencePath);
const sourceDigest = createHash('sha256').update(sourceBytes).digest('hex');
if (sourceDigest !== evidenceDigest[1]) throw new Error('publicMiningActivation digest does not match exact sourceCommit blob bytes');
if (createHash('sha256').update(workingBytes).digest('hex') !== sourceDigest) throw new Error('publicMiningActivation working-tree evidence differs from exact sourceCommit blob');
const otherDigests = new Set(Object.entries(policy.evidence || {})
  .filter(([name]) => name !== 'publicMiningActivation')
  .map(([, value]) => typeof value === 'string' ? value.match(digestFragment)?.[1] : null)
  .filter(Boolean));
if (artifactDigests.has(sourceDigest) || subjects.some((s) => s.sbom.sha256 === sourceDigest) || otherDigests.has(sourceDigest)) throw new Error('publicMiningActivation byte identity must not alias artifact, SBOM, or another evidence role');

let document;
try { document = JSON.parse(sourceBytes.toString('utf8')); } catch { throw new Error('publicMiningActivation evidence must be valid JSON'); }
requireExactKeys(document, ['schemaVersion', 'releaseVersion', 'subjects', 'verification'], 'publicMiningActivation evidence');
if (document.schemaVersion !== 3) throw new Error('publicMiningActivation evidence must use schemaVersion 3');
if (document.releaseVersion !== policy.releaseVersion) throw new Error('publicMiningActivation release identity mismatch');
if (!Array.isArray(document.subjects) || JSON.stringify(document.subjects) !== JSON.stringify(subjects)) throw new Error('publicMiningActivation subjects do not match exact promoted artifact/SBOM subjects');
for (const [index, subject] of document.subjects.entries()) {
  requireExactKeys(subject, ['platform', 'artifact', 'sbom'], `publicMiningActivation subject ${index}`);
  requireExactKeys(subject.artifact, ['name', 'sha256'], `publicMiningActivation artifact ${index}`);
  requireExactKeys(subject.sbom, ['name', 'sha256'], `publicMiningActivation SBOM ${index}`);
}
requireExactKeys(document.verification, ['verified', 'method', 'tool'], 'publicMiningActivation verification');
if (document.verification.verified !== true) throw new Error('publicMiningActivation verification must be explicitly true');
if (!allowedMethods.has(document.verification.method)) throw new Error('publicMiningActivation evidence uses an unapproved verification method');
if (typeof document.verification.tool !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9 ._+:/-]{1,127}$/.test(document.verification.tool)) throw new Error('publicMiningActivation evidence requires a bounded verification tool identity');

runGit(['cat-file', '-e', `${policy.sourceCommit}^{commit}`]);
console.log('public-mining activation evidence is byte-bound to exact sourceCommit with explicit positive verification');
