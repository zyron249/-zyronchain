...
function expectStatus(result, allowed, id) {
  if (!allowed.includes(result.status)) finding("high", id, `unexpected HTTP ${result.status}`);
}

const baseline = await request("/status");
assert.equal(baseline.status, 200, `baseline /status returned ${baseline.status}`);
assert.equal(typeof baseline.body, "object");

for (const [field, expected] of [
  ["valueBearing", false],
  ["publicTestnetAuthorized", false],
  ["mainnetAuthorized", false],
  ["validatorProcesses", 4],
  ["failureDomains", 1]
]) {
...
for (const path of ["/", "/readyz", "/healthz"]) {
  ...
  expectStatus(result, [200, 503], `baseline-${path}`);
}
...
for (const path of hiddenPaths) {
  ...
  if (![404, 405].includes(result.status)) {
    finding("high", "hidden-path-exposure", `${path} returned HTTP ${result.status}`);
  }
  if (/root:.*:0:0:|PRIVATE KEY|privateKey|PASSWORD=/i.test(result.text)) {
    finding("critical", "sensitive-file-disclosure", `${path} returned sensitive-looking content`);
  }
}

const writePaths = ["/block", "/tx", "/proposal/attest", "/round/skip", "/validator", "/protocol"];
...
for (const method of ["POST", "PUT", "PATCH", "DELETE", "OPTIONS", "TRACE", "HEAD"]) {
  const result = await request("/status", { method });
  if (result.status !== 405) finding("high", "method-confusion", `${method} /status returned ${result.status}`);
}

const spoof = await request("/status", {
  headers: {
...
if (JSON.stringify(spoof.body) !== baselineText) {
  observations.push({ id: "spoofed-header-response-differs", detail: "Spoofed proxy/auth headers changed the public status representation; manually review if intentional." });
}

const oversized = await request("/block", {
  method: "POST",
  headers: { "content-type": "application/json" },
  body: JSON.stringify({ junk: "A".repeat(64 * 1024) })
});
if (oversized.status !== 405) finding("high", "oversized-write-body", `64KiB POST /block returned ${oversized.status}`);

const longQuery = await request(`/status?probe=${"a".repeat(4096)}`);
if (longQuery.status !== 200) finding("medium", "long-query-availability", `4KiB query returned ${longQuery.status}`);

const burst = await Promise.all(Array.from({ length: 24 }, () => request("/status")));
...
assert.equal(after.status, 200, `post-probe /status returned ${after.status}`);
if (after.body?.genesisHash !== baselineGenesis) finding("critical", "genesis-changed-during-probe", "genesis changed during non-destructive probe");
