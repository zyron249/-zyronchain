...
// release PR may replace null asset URLs and digests only after independently
...
if (miningStart && document.title.includes('Mining Launchpad')) {
  ...
  const assetSha256 = detectedPlatform === 'unknown' ? null : MINER_DISTRIBUTION.assetSha256[detectedPlatform];
  ...
  const trustedSha256 = typeof assetSha256 === 'string' && /^[0-9a-f]{64}$/.test(assetSha256);
  const live = MINER_DISTRIBUTION.enabled === true && MINER_DISTRIBUTION.publicMiningActivated === true && trustedReleaseAsset && trustedSha256;
  ...
  digest.className = 'mining-note';
  digest.textContent = live ? `Expected SHA-256: ${assetSha256}` : 'Expected SHA-256 will be published only with an activated reviewed release.';
  ...
  shell.append(head, description, button, digest, privacy);
