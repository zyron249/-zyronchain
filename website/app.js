...
// The public website prepares a one-click miner entrypoint but fails closed until
// both signed release assets and protocol-v5 public mining activation are explicit.
...
if (miningStart && document.title.includes('Mining Launchpad')) {
  ...
  const isTrustedAsset = typeof asset === 'string' && /^https:\/\/github\.com\/zyron249\/-zyronchain\/releases\/download\/[^/]+\/ZyronMiner-[A-Za-z0-9._-]+$/.test(asset);
  const live = MINER_DISTRIBUTION.enabled === true && MINER_DISTRIBUTION.publicMiningActivated === true && isTrustedAsset;

  const shell = document.createElement('div');
  shell.className = 'shell command-card';
  shell.setAttribute('aria-label', 'One-click miner download');
  ...
  description.textContent = live
    ? `Download the signed ZyronMiner package for ${platformLabel}. Open it once; miner custody and network configuration stay local.`
    : `One-click ${platformLabel} download is prepared but remains disabled until signed release assets exist and public mining is explicitly activated.`;
  ...
  privacy.textContent = 'The website never requests a private key, seed phrase, wallet password or browser mining permission. Downloaded software never auto-executes without operating-system/user consent.';
