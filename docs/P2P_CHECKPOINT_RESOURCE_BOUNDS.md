...
The parsed snapshot object still overlaps the canonical serialization needed for canonical-JSON verification, and trusted-snapshot reconstruction can itself require substantial target-scale memory. The lexical complexity bound constrains parser amplification but does not prove target-scale peak-memory safety. These controls therefore reduce transient recovery peak memory but do **not** constitute target-hardware capacity evidence and do not close the State-v2 capacity/recovery STOP-SHIP gate tracked by #383.
