...
ZyronChain persists HTTP peer and native libp2p reputation so active penalties survive restart. Logical cardinality remains capped at 256 entries, persisted byte size is bounded before UTF-8/JSON materialization, and successful HTTP reputation publication includes directory-entry durability.
...
- HTTP reputation publication writes an owner-only temporary file, fsyncs it, atomically renames it and fsyncs the parent directory before reporting success.
- A pre-rename HTTP publication failure removes the unpublished temporary file. A failure after rename is surfaced to the caller rather than being reported as a durably completed mutation.
...
- `l1/test/peer-reputation.test.ts` covers oversized HTTP reputation state, endpoint byte limits, temporary cleanup, post-rename fault surfacing, directory-sync completion and restart behavior.
