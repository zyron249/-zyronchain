...
- the primary mutable read buffer is zeroized in `finally` before the descriptor is closed, including validation/error paths, and the bounded hard-link reread buffer is likewise zeroized in its own `finally` path after digest comparison;
...
The published `zyron-l1` entrypoint (`secure-cli.ts`) explicitly runs the canonical CLI security policy before any command staging or delegation. That policy therefore fail-closes passwordless new-key generation, insecure remote RPC URLs and unsafe private-file preflight at the actual public entrypoint rather than depending on a later import side effect. `rpc-client.ts` retains the same enforcement when `cli.ts` is loaded, providing a second defense-in-depth check rather than weakening or replacing the entrypoint boundary.
...
Operator authentication-token files are also representation-bound. The canonical form is exactly one 32-512 byte printable ASCII token (`0x21` through `0x7e`), optionally followed by one final LF or CRLF line terminator. Leading/trailing spaces or tabs, embedded line breaks, and repeated trailing line terminators are rejected instead of being silently normalized. This keeps the credential bytes consumed by RPC/P2P authentication unambiguous while preserving editor-friendly single-line token files.

Encrypted-keystore operations also bound mutable in-process secret-buffer lifetime. The scrypt-derived AES-256 key buffer is zeroized in `finally` on both encryption and decryption paths, including authentication/error exits. Encryption's operation-scoped plaintext private-key byte buffer and decryption's operation-scoped plaintext output buffer are likewise overwritten once ciphering/validation has completed. Local-secret file reads now apply the same principle to the mutable descriptor-read buffers themselves. Returned JavaScript password/private-key/token strings remain subject to runtime garbage-collection behavior and cannot be reliably zeroized in place; these controls therefore reduce avoidable mutable-buffer residency rather than claiming HSM-equivalent memory secrecy.

These controls reduce local pre-open path-replacement, junction/reparse race, same-inode content-mutation, cross-account secret-custody, ambiguous key/token serialization, accidental-permission, oversized/repeated-read denial-of-service and avoidable mutable secret-buffer lifetime risk. They do not replace production HSM or independently audited remote-signer custody. Public-testnet and mainnet activation remain governed by the external evidence gates and must not be inferred from these local hardening controls.
