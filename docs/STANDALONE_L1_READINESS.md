...
| L | Ledger state | Implemented with gate | O(1) in-memory account/nonce state, deterministic replay/state root, and disk-backed finalized history with a compact byte-offset sync index keep full block objects out of long-lived RAM; authenticated snapshot/checkpoint acceleration remains |
