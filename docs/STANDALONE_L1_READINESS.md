...
| L | Ledger state | Implemented with gate | authenticated State v2, crash-safe node+semantic-key persistence, externally anchored portable state sync, pruning/checkpoint recovery and no duplicate legacy ledger shadow after v2 activation are implemented; current reachable sparse nodes are still fully hydrated in RAM, so indexed/lazy state access and scale evidence remain |
