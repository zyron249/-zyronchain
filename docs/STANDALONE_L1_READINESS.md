...
| L | Ledger state | Implemented with gate | O(1) in-memory account/nonce state, deterministic replay/state root, disk-backed finalized history with compact byte-offset sync index, and deterministic full-state/schedule checkpoint export with publishable SHA-256; fast import remains disabled until its complete snapshot digest is independently trusted |
