...
Active publication verification runs on a host supporting `O_NOFOLLOW` (the Linux promotion CI). It reads a maximum 64 KiB through the same no-follow descriptor it validates, rejects changed file/directory identities, and closes the descriptor on failure. Hosts without this primitive reject active publication, while the inactive policy remains portable. Windows-only checks cannot substitute for the Linux positive, substitution and symlink regressions.
