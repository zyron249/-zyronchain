...
- protocol-v5 permissionless mining tests remain present and the public mining mempool retains an explicit bounded subpool;
...
- the operations runbook, mining specification and threat model remain part of the required package;
...
- the external-audit handoff remains explicitly preparation-only and keeps independent mining calibration/retest, audit, Internet-soak and production-custody gates open;
- irreversible mainnet chain/genesis/mining-calibration/economic/governance decisions remain unresolved;
- the current permissioned PoA/BFT **finality architecture** is not silently relabeled as permissionless consensus;
- protocol-v5 proof of work remains an issuance mechanism and is not silently converted into Nakamoto-style chain-selection mining.
...
A green result means **internal private/adversarial testnet engineering preflight passed for the tested commit**. It is useful for handing a deterministic build/test/mining package to future independent operators without allowing readiness language to drift.
...
A green private preflight therefore does **not** mean public testnet, public mining or mainnet activation is allowed. It still carries unresolved external/operator/infrastructure/mining-calibration gates and unresolved mainnet-only decisions.
...
5. independent protocol-v5 mining review, heterogeneous-hardware contention, stale-tip/inclusion-censorship evidence, target calibration and independent retest;
6. sustained adversarial Internet soak with independent operators and real reviewed always-on duration evidence;
7. production HSM/audited signer custody and cross-host key rotation;
8. multi-region disaster-recovery/incident drills;
9. repository branch-protection and independent-review settings enforced by the hosting platform;
10. at least two genuinely independent maintainer/security/release custodians plus rehearsed domain/checkpoint succession.

Mainnet additionally requires explicit human/governance decisions for immutable chain identity, zero-premine or otherwise explicitly disclosed genesis allocation, final mining target/retarget calibration, validator economics, oracle governance and validator admission/removal.
