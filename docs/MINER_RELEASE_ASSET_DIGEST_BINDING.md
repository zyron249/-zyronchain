...
Promoted platform assets must also be pairwise distinct and their basenames must carry the canonical marker for the declared slot: `-windows-`, `-macos-`, or `-linux-`. This prevents one immutable artifact from being aliased across multiple OS slots and prevents an OS-selected website CTA from being approved against an artifact named for a different platform. This naming check is an additional publication-integrity boundary; it does not replace platform signing, notarization, checksum verification, provenance, or runtime validation.
