...
Outbound framing keeps the same conservative allocation gate but now separates peak allocation custody from slow-reader retention. Before `JSON.stringify` or UTF-8 Buffer creation, the writer still reserves two full frame-sized allowances from the process-wide outbound budget: one for the transient serialized JS string and one for the encoded Buffer. After the encoded body has been materialized and validated against `maxBytes`, the transient serialization allowance is released immediately and the encoded allowance is reduced to the body's actual byte length before any asynchronous send, drain wait, or stream-close boundary. The actual encoded-body reservation remains held until the send boundary completes and is released on success, timeout, backpressure failure, or close failure. Serialization and oversize failures release both conservative pre-allocation reservations without entering the asynchronous send path.

This distinction prevents a small frame under a large protocol ceiling from occupying two ceiling-sized outbound reservations for the lifetime of a slow peer, while preserving the fail-closed two-ceiling admission gate around serialization/encoding itself. It improves outbound availability accounting without weakening protocol-specific frame ceilings or allowing an allocation before budget admission.
