...
- Public RPC availability assumes layered perimeter controls; the node additionally bounds individual bodies, aggregate request-body bytes retained conservatively through handler completion, rates, total connections, node-wide in-flight requests, header parsing, request/header/keep-alive time and requests per persistent socket. Overload is shed with `503` before routing or body work begins; current occupancy, configured capacity and cumulative admission rejections are exposed to local monitoring.
