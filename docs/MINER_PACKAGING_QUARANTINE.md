...
A POSIX descriptor-relative custody foundation exists under `l1/native/miner-custody-posix.c`. It uses `open(..., O_DIRECTORY|O_NOFOLLOW)`, `openat`, and `mkdirat` with exclusive child creation semantics. The helper now also exposes a long-lived `session` mode that binds the release-root descriptor once and retains that same descriptor across multiple child operations instead of reopening the release pathname for every operation. The adversarial POSIX probe renames the bound release root, replaces the original pathname with a symlink to an external sentinel directory, then proves a subsequent session write still lands on the held descriptor and writes zero candidate bytes through the replacement pathname.

Miner Package / Miner Release Candidate CI compile and exercise the primitive on Linux and macOS. Windows explicitly treats this POSIX primitive as unsupported and remains protected by the packaging quarantine. The retained-descriptor session is still only a custody primitive: `package-miner.mjs` has not yet moved candidate materialization into that session, nested destination traversal is not yet implemented through retained child descriptors, and this work therefore does **not** satisfy or close #761 by itself.
