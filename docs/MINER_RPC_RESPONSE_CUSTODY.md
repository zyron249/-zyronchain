...
Cleanup is intentionally subordinate to validation: a body-cancellation failure never converts a rejected response into an accepted one, never replaces the original protocol or size error, and an unresolved cancellation promise is not awaited before the rejection is returned. This prevents cleanup itself from becoming an attacker-controlled delay on the fail-closed path. Existing HTTPS/loopback restrictions, redirect rejection, 64 KiB response limit, JSON depth and structural-token limits, chain/genesis identity checks, protocol-v5 activation gating, local encrypted key custody and mining-claim validation remain unchanged.
