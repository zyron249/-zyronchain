...
const requireExactRef = (action, expected) => {
  ...
  if (refs.length !== 1 || refs[0] !== expected) throw new Error(`Launch authorization ${action} pin drift`);
  if (new RegExp(`actions/${action}@(?![0-9a-f]{40}\\b)[^\\s]+`).test(workflow)) throw new Error(`Mutable ${action} ref in launch-authorization workflow`);
};
...
if (!/^\s*persist-credentials:\s*false\b/m.test(checkoutStep)) throw new Error('launch-authorization checkout must disable credential persistence');
...
for (const required of [
  ...
  if (!workflow.includes(required)) throw new Error(`Required launch-authorization security invariant missing: ${required}`);
}

console.log('l1-launch-authorization-action-custody: ok');
