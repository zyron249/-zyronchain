...
function extractPlatformObject(name) {
  ...
  const parse = (raw) => {
    ...
    throw new Error(`website miner ${name} value must be a static string/null literal: ${value}`);
  };
...
const website = {
  ...
  assets: extractPlatformObject('assets'),
  assetSha256: extractPlatformObject('assetSha256')
};
...
for (const platform of requiredPlatforms) {
  ...
  if (typeof canonicalDigest !== 'string' || !trustedSha256.test(canonicalDigest)) throw new Error(`canonical ${platform} asset digest is not a lowercase SHA-256`);
